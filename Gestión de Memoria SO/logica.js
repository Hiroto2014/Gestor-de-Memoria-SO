...
function llenarProgramas() { //Se llena la tabla de programas con los programas precargados
    document.getElementById("programas").replaceChildren();
    for (let i = 0; i < programas.length; i++) {
        const programa = programas[i];

        var fila = "<tr><td>" + programa.nombre 
        + "</td><td>" + programa.text
        + "</td><td>" + programa.data
        + "</td><td>" + calcularBSS(programa.tamano, programa.text, programa.data)
        + "</td><td>" + programa.tamano 
        + "</td><td>" + programa.mem_use
        + "</td><td>" + calcularKib(programa.mem_use) 
        + "</td><td><button name = 'btnEncender' class='btn btnEncender'" + " value='" + i + "' disabled>Encender</button>" + "</td></tr>";
...
function calcularKib (mem_use){
    return (mem_use/1024).toFixed(2);
}
function calcularBSS(tamano, text, data){
    return (tamano-text-data)
}

function init() {
    llenarProgramas();
    agregarListener();
    $(".ordenamiento").hide(); //Ocultar Ordenamientos en primera Carga
}
